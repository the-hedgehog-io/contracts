...
class TestHelper {
  ...
  // Adds the gas compensation (50 BaseFeeLMA)
  ...
  /*
   * given the requested BaseFeeLMA amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, baseFeeLMAAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(baseFeeLMAAmount);
    const compositeDebt = await this.getCompositeDebt(
      contracts,
      baseFeeLMAAmount
    );
  ...
  /*
   * given the desired total debt, returns the BaseFeeLMA amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveBaseFeeLMAAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, baseFeeLMAAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(baseFeeLMAAmount);
    return baseFeeLMAAmount.add(fee);
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        const BaseFeeLMAAmount = redemptionTx.logs[i].args[0];
        const totalBaseFeeLMARedeemed = redemptionTx.logs[i].args[1];
        const totalETHDrawn = redemptionTx.logs[i].args[2];
        const ETHFee = redemptionTx.logs[i].args[3];

        return [
          BaseFeeLMAAmount,
          totalBaseFeeLMARedeemed,
          totalETHDrawn,
          ETHFee,
        ];
      }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const baseFeeLMAGasComp = liquidationTx.logs[i].args[3];

        return [liquidatedDebt, liquidatedColl, collGasComp, baseFeeLMAGasComp];
      }
    }
  ...
  static getBaseFeeLMAFeeFromBaseFeeLMABorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "BaseFeeLMABorrowingFeePaid") {
        return tx.logs[i].args[1].toString();
      }
    }
    throw "The transaction logs do not contain an BaseFeeLMABorrowingFeePaid event";
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const entireDebt = rawDebt.add(pendingBaseFeeLMADebtReward);
  ...
  static async getCollAndDebtFromWithdrawBaseFeeLMA(
    contracts,
    account,
    amount
  ) {
  ...
  static async getCollAndDebtFromRepayBaseFeeLMA(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(
      contracts,
      account
    );
  ...
  static async getCollAndDebtFromAdjustment(
    ...
    const fee = BaseFeeLMAChange.gt(this.toBN("0"))
      ? await contracts.troveManager.getBorrowingFee(BaseFeeLMAChange)
      : this.toBN("0");
    const newColl = entireColl.add(ETHChange);
    const newDebt = entireDebt.add(BaseFeeLMAChange).add(fee);
  ...
  static async openTrove_allAccounts_randomETH_ProportionalBaseFeeLMA(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH);
      const proportionalBaseFeeLMA = web3.utils
        .toBN(proportion)
        .mul(web3.utils.toBN(randCollAmount));
      const totalDebt = await this.getOpenTroveTotalDebt(
        contracts,
        proportionalBaseFeeLMA
      );
  ...
  static async openTrove_allAccounts_randomETH_randomBaseFeeLMA(
    minETH,
    maxETH,
    accounts,
    contracts,
    minBaseFeeLMAProportion,
    maxBaseFeeLMAProportion,
    logging = false
  ) {
    const gasCostList = [];
    const price = await contracts.priceFeedTestnet.getPrice();
    ...
    for (const account of accounts) {
      ...
      const randBaseFeeLMAProportion = this.randAmountInWei(
        minBaseFeeLMAProportion,
        maxBaseFeeLMAProportion
      );
      const proportionalBaseFeeLMA = web3.utils
        .toBN(randBaseFeeLMAProportion)
      ...
      if (logging && tx.receipt.status) {
        i++;
        const ICR = await contracts.troveManager.getCurrentICR(account, price);
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalBaseFeeLMA} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
  ...
  static async openTrove_allAccounts_randomBaseFeeLMA(
    minBaseFeeLMA,
    maxBaseFeeLMA,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randBaseFeeLMAAmount = this.randAmountInWei(
        minBaseFeeLMA,
        maxBaseFeeLMA
      );
      const totalDebt = await this.getOpenTroveTotalDebt(
        contracts,
        randBaseFeeLMAAmount
      );
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        ETHAmount,
  ...
  static async openTrove_allAccounts_decreasingBaseFeeLMAAmounts(
    accounts,
    contracts,
    ETHAmount,
    maxBaseFeeLMAAmount
  ...
  static async openTrove(
    ...
    if (!extraBaseFeeLMAAmount) extraBaseFeeLMAAmount = this.toBN(0);
    else if (typeof extraBaseFeeLMAAmount == "string")
      extraBaseFeeLMAAmount = this.toBN(extraBaseFeeLMAAmount);
    ...
    const baseFeeLMAAmount = MIN_DEBT.add(extraBaseFeeLMAAmount);
  ...
  static async withdrawBaseFeeLMA(
    contracts,
    {
      maxFeePercentage,
      baseFeeLMAAmount,
    ...
    assert(
      !(baseFeeLMAAmount && ICR) && (baseFeeLMAAmount || ICR),
      "Specify either baseFeeLMA amount or target ICR, but not both"
    );
    ...
    if (ICR) {
      ...
      baseFeeLMAAmount = await this.getNetBorrowingAmount(
        contracts,
        increasedTotalDebt
      );
    } else {
    ...
    await contracts.borrowerOperations.withdrawBaseFeeLMA(
      maxFeePercentage,
      baseFeeLMAAmount,
      upperHint,
      lowerHint,
  ...
  static async adjustTrove_allAccounts(
    ...
    for (const account of accounts) {
      ...
      let BaseFeeLMAChangeBN = this.toBN(BaseFeeLMAAmount);
  ...
  static async adjustTrove_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      ...
      let BaseFeeLMAChangeBN = this.toBN(
        this.randAmountInWei(BaseFeeLMAMin, BaseFeeLMAMax)
      );
  ...
  static async withdrawBaseFeeLMA_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } =
        await this.getCollAndDebtFromWithdrawBaseFeeLMA(
          contracts,
          account,
          amount
        );
      ...
      const tx = await contracts.borrowerOperations.withdrawBaseFeeLMA(
        this._100pct,
        amount,
        upperHint,
        lowerHint,
  ...
  static async withdrawBaseFeeLMA_allAccounts_randomAmount(
    min,
    max,
    accounts,
    contracts
    ...
    for (const account of accounts) {
      const randBaseFeeLMAAmount = this.randAmountInWei(min, max);
  ...
  static async repayBaseFeeLMA_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayBaseFeeLMA(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.repayBaseFeeLMA(
        amount,
        upperHint,
        lowerHint,
        { from: account }
  ...
  static async repayBaseFeeLMA_allAccounts_randomAmount(
    min,
    max,
    accounts,
    contracts
  ...
  static async provideToSP_allAccounts_randomAmount(
    ...
    const gasCostList = [];
    for (const account of accounts) {
      const randomBaseFeeLMAAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.provideToSP(
        randomBaseFeeLMAAmount,
        this.ZERO_ADDRESS,
        { from: account }
      );
      const gas = this.gasUsed(tx);
  ...
  static async withdrawFromSP_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      const randomBaseFeeLMAAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.withdrawFromSP(randomBaseFeeLMAAmount, {
        from: account,
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async getTimeFromSystemDeployment(
    ...
    const deploymentTime = await hogToken.getDeploymentStartTime();
