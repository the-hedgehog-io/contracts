...
contract("HintHelpers", async (accounts) => {
  ...
  /* Open a Trove for each account. BaseFeeLMA debt is 200 BaseFeeLMA each, with collateral beginning at
  1.5 eth, and rising by 0.01 eth per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

  // Open Troves in parallel, then withdraw BaseFeeLMA in parallel
  const makeTrovesInParallel = async (accounts, n) => {
    ...
    const withdrawBaseFeeLMApromises = activeAccounts.map((account) =>
      withdrawBaseFeeLMAfromTrove(account)
    );
    await Promise.all(withdrawBaseFeeLMApromises);
  ...
  const withdrawBaseFeeLMAfromTrove = async (account) => {
  ...
  // Sequentially add coll and withdraw BaseFeeLMA, 1 account at a time
  ...
  it("getApproxHint(): returns the address of a Trove within sqrt(length) positions of the correct insert position", async () => {
    ...
    const CRPercent_250 = Number(web3.utils.fromWei(CR_250, "eth")) * 100;
    ...
    const ICRPercent_hintAddress_250 =
      Number(web3.utils.fromWei(ICR_hintAddress_250, "eth")) * 100;
    ...
    const CRPercent_287 = Number(web3.utils.fromWei(CR_287, "eth")) * 100;
    ...
    const ICRPercent_hintAddress_287 =
      Number(web3.utils.fromWei(ICR_hintAddress_287, "eth")) * 100;
    ...
    const CRPercent_213 = Number(web3.utils.fromWei(CR_213, "eth")) * 100;
    ...
    const ICRPercent_hintAddress_213 =
      Number(web3.utils.fromWei(ICR_hintAddress_213, "eth")) * 100;
    ...
    const CRPercent_201 = Number(web3.utils.fromWei(CR_201, "eth")) * 100;
    ...
    const ICRPercent_hintAddress_201 =
      Number(web3.utils.fromWei(ICR_hintAddress_201, "eth")) * 100;
  ...
  //     const ICRPercent_hintAddress = Number(web3.utils.fromWei(ICR_hintAddress, 'eth')) * 100
  ...
  it("getApproxHint(): returns the head of the list if the CR is the max uint256 value", async () => {
    ...
    const ICRPercent_hintAddress_Max =
      Number(web3.utils.fromWei(ICR_hintAddress_Max, "eth")) * 100;
    ...
    const ICRPercent_FirstTrove =
      Number(web3.utils.fromWei(ICR_FirstTrove, "eth")) * 100;
  ...
  it("getApproxHint(): returns the tail of the list if the CR is lower than ICR of any Trove", async () => {
    ...
    const ICRPercent_hintAddress_Min =
      Number(web3.utils.fromWei(ICR_hintAddress_Min, "eth")) * 100;
    ...
    const ICRPercent_LastTrove =
      Number(web3.utils.fromWei(ICR_LastTrove, "eth")) * 100;
