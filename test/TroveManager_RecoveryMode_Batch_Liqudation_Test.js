  ...
  async (accounts) => {
    ...
    context("Batch liquidations", () => {
      ...
      it("Stability Pool profit matches", async () => {
        ...
        const expectedGainInBaseFeeLMA = expectedCollateralLiquidatedA
          .mul(price)
          .div(mv._1e18BN)
          .sub(A_totalDebt);
        const realGainInBaseFeeLMA = spEthAfter
          .sub(spEthBefore)
          .mul(price)
          .div(mv._1e18BN)
        ...
        assert.equal(
          ...
          "Stability Pool BaseFeeLMA doesn’t match"
        );
        assert.equal(
          realGainInBaseFeeLMA.toString(),
          expectedGainInBaseFeeLMA.toString(),
