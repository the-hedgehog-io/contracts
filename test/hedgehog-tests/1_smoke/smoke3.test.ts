  ...
  context("Base functionality and Access Control . Flow #3", () => {
    ...
    it("should let provide to stability pool in recovery mode", async () => {
      await expect(
        provideToStabilityPool({
          caller: carol,
          amount: "67975554076726728500000000",
        })
      ).to.be.not.reverted;
    });
    ...
    it("should have both positions closed", async () => {
      await expect(
        getTrove({
          owner: bob,
          expectedColl: "2528977540106951870400",
          expectedDebt: "6833212299465240641711229600",
        })
      ).not.to.be.reverted;
      await expect(
        getTrove({
      ...
      await expect(
        getTrove({
          owner: carol,
          expectedColl: "1412012459893048127640",
          expectedDebt: "1757087700534759358288769860",
        })
      ).not.to.be.reverted;
    });
  });
