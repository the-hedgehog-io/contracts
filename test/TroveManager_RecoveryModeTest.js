...
contract("TroveManager - in Recovery Mode", async (accounts) => {
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1ETH:150BaseFeeLMA, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("checkRecoveryMode(): returns false if TCR stays above CCR", async () => {
    ...
    await borrowerOperations.withdrawColl(_1_Ether, alice, alice, {
      from: alice,
    });
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100BaseFeeLMA, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 BaseFeeLMA of debt, bringing his ICR to 210%
    ...
    // --- TEST ---
    // price drops to 1ETH:100BaseFeeLMA, reducing TCR to 117%
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 BaseFeeLMA of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(150, 16)),
      extraParams: { from: alice },
    });
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(210, 16)),
      extraBaseFeeLMAAmount: dec(20000, 18),
      extraParams: { from: bob },
    });
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(150, 16)),
    ...
    /*
    Prior to Dennis liquidation, total stakes and total collateral were each 27 eth. 
  
    Check snapshots. Dennis' liquidated collateral is distributed and remains in the system. His 
    stake is removed, leaving 24+3*0.995 eth total collateral, and 24 eth total stakes. */
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390BaseFeeLMA to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice });
    ...
    // check pool BaseFeeLMA before liquidation
    const stabilityPoolBaseFeeLMA_Before = (
      await stabilityPool.getTotalBaseFeeLMADeposits()
    ).toString();
    assert.equal(stabilityPoolBaseFeeLMA_Before, "390000000000000000000");
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 eth, and liquidated debt is 2000 BaseFeeLMA.
    
    With 390 BaseFeeLMA in the StabilityPool, 390 BaseFeeLMA should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    BaseFeeLMALoss: 390BaseFeeLMA
    ETHGain: (390 / 2000) * 21*0.995 = 4.074525 eth

    After offsetting 390 BaseFeeLMA and 4.074525 eth, the remainders - 1610 BaseFeeLMA and 16.820475 eth - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner });
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 BaseFeeLMA and 16.82 eth are distributed.
    
    Now, only Alice and Dennis have a stake in the system - 3 eth each, thus total stakes is 6 eth.
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool BaseFeeLMA is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraBaseFeeLMAAmount: dec(250, 18),
      extraParams: { from: bob },
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Alice deposits BaseFeeLMA in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 BaseFeeLMA, Alice sole depositor.
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(220, 16)),
      extraBaseFeeLMAAmount: dec(250, 18),
      extraParams: { from: bob },
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
    ...
    // Alice deposits BaseFeeLMA in the Stability Pool
    await stabilityPool.provideToSP(B_totalDebt.add(toBN(1)), ZERO_ADDRESS, {
      from: alice,
    });
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
    ...
    // totalStakesSnapshot should have reduced to 22 eth - the sum of Alice's coll( 20 eth) and Dennis' coll (2 eth )
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 BaseFeeLMA, CR of 200%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    });
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraBaseFeeLMAAmount: totalLiquidatedDebt,
  ...
  BaseFeeLMA is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool BaseFeeLMA < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraBaseFeeLMAAmount: dec(1500, 18),
      extraParams: { from: alice },
    });
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(240, 16)),
    ...
    // Alice deposits 1490 BaseFeeLMA in the Stability Pool
    await stabilityPool.provideToSP("1490000000000000000000", ZERO_ADDRESS, {
      from: alice,
    });
    ...
    /* Since the pool only contains 100 BaseFeeLMA, and Bob's pre-liquidation debt was 250 BaseFeeLMA,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool BaseFeeLMA < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    // Alice deposits 100 BaseFeeLMA in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool BaseFeeLMA < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 BaseFeeLMA) is larger than all BaseFeeLMA in the Stability Pool, Liquidation won’t happen

    After liquidation, totalStakes snapshot should equal Alice's stake (20 eth) + Dennis stake (2 eth) = 22 eth.

    Since there has been no redistribution, the totalCollateral snapshot should equal the totalStakes snapshot: 22 eth.

    Bob's new coll and stake should remain the same, and the updated totalStakes should still equal 25 eth.
    */
    const bob_Trove = await troveManager.Troves(bob);
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool BaseFeeLMA < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /* After liquidation, totalStakes snapshot should still equal the total stake: 25 eth

    Since there has been no redistribution, the totalCollateral snapshot should equal the totalStakes snapshot: 25 eth.*/
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool BaseFeeLMA < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 BaseFeeLMA, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraBaseFeeLMAAmount: dec(1500, 18),
      extraParams: { from: alice },
    ...
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(250, 16)),
      extraBaseFeeLMAAmount: dec(240, 18),
      extraParams: { from: carol },
    });
    ...
    //Check SP BaseFeeLMA has been completely emptied
    assert.equal(
    ...
    await baseFeeLMAToken.transfer(bob, dec(100, 18), { from: alice });
    ...
    // Alice provides another 50 BaseFeeLMA to pool
  ...
  it("liquidate() with ICR > 110%, and StabilityPool BaseFeeLMA < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 BaseFeeLMA to the SP
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraBaseFeeLMAAmount: dec(50, 18),
      extraParams: { from: whale },
    });
  ...
  it("liquidate(): does nothing if trove ICR >= TCR, and SP covers trove's debt", async () => {
    ...
    // C fills SP with 130 BaseFeeLMA
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 BaseFeeLMA in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or StETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    //Dennis provides 200 BaseFeeLMA to SP
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({
      ICR: toBN(dec(220, 16)),
      extraBaseFeeLMAAmount: dec(1000, 18),
      extraParams: { from: whale },
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 BaseFeeLMA of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 BaseFeeLMA of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraBaseFeeLMAAmount: dec(480, 18),
      extraParams: { from: bob },
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraBaseFeeLMAAmount: B_netDebt_2,
      extraParams: { from: dennis },
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 BaseFeeLMA of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraBaseFeeLMAAmount: dec(90, 18),
      extraParams: { from: bob },
    });
    // Dennis withdraws to 150 BaseFeeLMA of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 250 BaseFeeLMA, ICR = 240% (lowest one)
    ...
    await openTrove({
      ICR: toBN(dec(266, 16)),
      extraBaseFeeLMAAmount: B_totalDebt_2,
      extraParams: { from: alice },
    });
    await stabilityPool.provideToSP(B_totalDebt_2, ZERO_ADDRESS, {
      from: alice,
    });
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some BaseFeeLMA from their Trove, resulting in different ICRs
    ...
    const liquidationAmount = E_totalDebt.add(F_totalDebt)
      .add(G_totalDebt)
      .add(H_totalDebt);
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraBaseFeeLMAAmount: liquidationAmount,
      extraParams: { from: alice },
    });

    // Alice deposits BaseFeeLMA to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, {
      from: alice,
    });

    // price drops
    // price drops to 1ETH:90BaseFeeLMA, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1ETH:85BaseFeeLMA, reducing TCR below 150%
  ...
  it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    assert.isTrue(
      (await troveManager.getCurrentICR(freddy, price)).lte(mv._MCR)
    );
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraBaseFeeLMAAmount: dec(101, 18),
      extraParams: { from: defaulter_1 },
    });
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraBaseFeeLMAAmount: dec(217, 18),
      extraParams: { from: defaulter_2 },
    });
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraBaseFeeLMAAmount: dec(328, 18),
      extraParams: { from: defaulter_3 },
    });
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraBaseFeeLMAAmount: dec(431, 18),
    ...
    assert.isTrue(
      await th.ICRbetween100and110(defaulter_3, troveManager, price)
    );
    assert.isTrue(
      await th.ICRbetween100and110(defaulter_4, troveManager, price)
    );
    ...
    // Check Stability Pool has 500 BaseFeeLMA
    assert.equal(
      (await stabilityPool.getTotalBaseFeeLMADeposits()).toString(),
      dec(500, 18)
    );
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    ...
    const totalCollAfter = W_coll.add(A_coll)
      ...
      .add(D_coll)
      .add(
        th.applyLiquidationFee(d1_coll.add(d2_coll).add(d3_coll).add(d4_coll))
      );
    ...
    assert.isAtMost(
      th.getDifference(
        TCR_After,
        totalCollAfter.mul(price).div(totalDebtAfter)
      ),
      1000
    );
    assert.isTrue(TCR_Before.gte(TCR_After));
    assert.isTrue(
      TCR_After.gte(TCR_Before.mul(th.toBN(995)).div(th.toBN(1000)))
    );
  });
  ...
  it("liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty", async () => {
    ...
    // Confirm 0 BaseFeeLMA in Stability Pool
    assert.equal(
      (await stabilityPool.getTotalBaseFeeLMADeposits()).toString(),
      "0"
    );
  ...
  it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    // Confirm BaseFeeLMA in Stability Pool
    assert.equal(
      (await stabilityPool.getTotalBaseFeeLMADeposits()).toString(),
      spDeposit.toString()
    );
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { baseFeeLMAAmount: baseFeeLMAAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis },
    });
    const { baseFeeLMAAmount: baseFeeLMAAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin },
    });
    const { baseFeeLMAAmount: baseFeeLMAAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy },
    });
    ...
    assert.equal(
      (await baseFeeLMAToken.balanceOf(freddy)).toString(),
      baseFeeLMAAmountF
    );
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and StETH gain", async () => {
    ...
    const { baseFeeLMAAmount: W_baseFeeLMAAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraBaseFeeLMAAmount: dec(4000, 18),
      extraParams: { from: whale },
    });
    await stabilityPool.provideToSP(W_baseFeeLMAAmount, ZERO_ADDRESS, {
      from: whale,
    });

    const {
      baseFeeLMAAmount: A_baseFeeLMAAmount,
    ...
    const {
      baseFeeLMAAmount: B_baseFeeLMAAmount,
    ...
    await stabilityPool.provideToSP(A_baseFeeLMAAmount, ZERO_ADDRESS, {
      from: alice,
    });
    await stabilityPool.provideToSP(B_baseFeeLMAAmount, ZERO_ADDRESS, {
      from: bob,
    });

    const totalDeposit =
      W_baseFeeLMAAmount.add(A_baseFeeLMAAmount).add(B_baseFeeLMAAmount);
    ...
    Total BaseFeeLMA in Pool: 680 BaseFeeLMA
    ...
    Bob BaseFeeLMA Loss: 500 * (240/680) = 176.47 BaseFeeLMA

    Whale remaining deposit: (400 - 294.12) = 105.88 BaseFeeLMA
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_baseFeeLMAAmount.sub(
          liquidatedDebt.mul(W_baseFeeLMAAmount).div(totalDeposit)
        )
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_baseFeeLMAAmount.sub(
          liquidatedDebt.mul(A_baseFeeLMAAmount).div(totalDeposit)
        )
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_baseFeeLMAAmount.sub(
          liquidatedDebt.mul(B_baseFeeLMAAmount).div(totalDeposit)
        )
      ),
      100000
    );
  ...
  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or StETH gain", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(182, 16)),
      extraBaseFeeLMAAmount: dec(170, 18),
      extraParams: { from: alice },
    });
    await openTrove({
      ICR: toBN(dec(180, 16)),
    ...
    assert.isTrue(
      (await troveManager.getCurrentICR(alice, price)).lte(mv._ICR100)
    );
    assert.isTrue(
      (await troveManager.getCurrentICR(bob, price)).lte(mv._ICR100)
    );
    assert.isTrue(
      (await troveManager.getCurrentICR(carol, price)).lte(mv._ICR100)
    );
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    That leaves 50 BaseFeeLMA in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10);
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraBaseFeeLMAAmount: D_totalDebt,
      extraParams: { from: carol },
    });
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } });
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 BaseFeeLMA in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 BaseFeeLMA in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10);

    const [liquidatedDebt, liquidatedColl, collGasComp, baseFeeLMAGasComp] =
      th.getEmittedLiquidationValues(liquidationTx);
    ...
    assert.equal(baseFeeLMAGasComp.toString(), dec(400, 18));
    ...
    th.assertIsApproximatelyEqual(
      alice_balanceAfter,
      alice_balanceBefore.add(
        th.toBN(alice_remainingCollateral).sub(th.toBN(ALICE_GAS * GAS_PRICE))
      )
    );
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await baseFeeLMAToken.transfer(alice, A_totalDebt, { from: whale });
    // Alice closes trove
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool BaseFeeLMA > debt to liquidate: can liquidate troves out of order", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice },
    });
    ...
    // Whale provides 1000 BaseFeeLMA to the SP
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(
      await troveManager.getPendingBaseFeeLMADebtReward(dennis)
    );
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    // Whale withdraws entire deposit, and re-deposits 132 BaseFeeLMA
    ...
    // Expected Stability Pool to fully absorb B (92 BaseFeeLMA + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalBaseFeeLMADeposits();
    ...
    const stabilityAfter = await stabilityPool.getTotalBaseFeeLMADeposits();
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    // Whale opens trove and adds 220 BaseFeeLMA to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(
      A_totalDebt.div(toBN(2))
    );
    await openTrove({
