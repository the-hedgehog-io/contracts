...
const Lqty = artifacts.require("HOGToken");
...
contract(
  ...
  function ([_, wallet1, wallet2, wallet3, wallet4, bountyAddress, owner]) {
    ...
    const deploy = async (that) => {
      ...
      const hogStaking = await NonPayable.new();
      const lockupContractFactory = await NonPayable.new();
      that.hog = await Lqty.new(
      ...
      that.lpRewardsEntitlement = await that.hog.getLpRewardsEntitlement();
      ...
      await that.uni.approve(that.pool.address, new BN(2).pow(new BN(255)), {
        from: wallet1,
      });
      await that.uni.approve(that.pool.address, new BN(2).pow(new BN(255)), {
        from: wallet2,
    ...
    describe("Unipool", async function () {
      ...
      it("Three stakers with the different (1:3:5) stakes wait different durations", async function () {
        ...
        expect(await this.pool.earned(wallet2)).to.be.bignumber.equal("0");
        expect(
          await this.hog.balanceOf(wallet2)
        ).to.be.bignumber.almostEqualDiv1e18(
      ...
      it("Four stakers with gaps of zero total supply", async function () {
        ...
        expect(await this.pool.periodFinish()).to.be.bignumber.equal(
        ...
        expect(await this.pool.earned(wallet1)).to.be.bignumber.equal("0");
        expect(
          await this.hog.balanceOf(wallet1)
        ).to.be.bignumber.almostEqualDiv1e18(
          rewardPerToken1.add(rewardPerToken2).mul(stake1).div(_1e18)
        );
        expect(
        ...
        expect(await this.pool.periodFinish()).to.be.bignumber.equal(
          stakeTime1.add(emptyPeriod1).add(this.DURATION)
        );
        ...
        expect(await this.pool.earned(wallet3)).to.be.bignumber.equal("0");
        expect(
          await this.hog.balanceOf(wallet3)
        ).to.be.bignumber.almostEqualDiv1e18(
        ...
        const emptyPeriod2 = stakeTime1
          .add(emptyPeriod1)
          .add(this.DURATION)
          .sub(exitTime3);
        expect(await this.pool.periodFinish()).to.be.bignumber.equal(
          stakeTime4.add(emptyPeriod2)
        );
        ...
        expect(await this.pool.earned(wallet3)).to.be.bignumber.equal("0");
        expect(
          await this.pool.earned(wallet4)
        ).to.be.bignumber.almostEqualDiv1e18(
    ...
    describe("Unipool, before calling setAddresses", async function () {
      ...
      it("Stake fails", async function () {
        const stake1 = new BN(web3.utils.toWei("1"));
        await assertRevert(
          this.pool.stake(stake1, { from: wallet1 }),
      ...
      it("Withdraw falis", async function () {
        const stake1 = new BN(web3.utils.toWei("1"));
        await assertRevert(
          this.pool.withdraw(stake1, { from: wallet1 }),
          "Liqudity Pool Token has not been set yet"
        );
      });
      ...
      it("Claim fails", async function () {
        await assertRevert(
          this.pool.claimReward({ from: wallet1 }),
          "Liqudity Pool Token has not been set yet"
        );
      });
      ...
      it("Exit fails", async function () {
        await assertRevert(
          this.pool.withdrawAndClaim({ from: wallet1 }),
          "Cannot withdraw 0"
        );
      });
    });
