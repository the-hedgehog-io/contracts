...
contract("Pool Manager: Sum-Product rounding errors", async (accounts) => {
  ...
  it.skip("Rounding errors: 100 deposits of 100BaseFeeLMA into SP, then 200 liquidations of 49BaseFeeLMA", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalBaseFeeLMADeposits();
    const SP_ETH = await stabilityPool.getWStETH();
    const compoundedDeposit =
      await stabilityPool.getCompoundedBaseFeeLMADeposit(depositors[0]);
    ...
    assert.isAtMost(
      th.getDifference(
        SP_TotalDeposits.div(th.toBN(depositors.length)),
        compoundedDeposit
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(SP_ETH.div(th.toBN(depositors.length)), ETH_Gain),
      100000
    );
  });
});
