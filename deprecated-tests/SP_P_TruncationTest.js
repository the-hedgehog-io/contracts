...
contract("StabilityPool Scale Factor issue tests", async (accounts) => {
  ...
  const getBaseFeeLMAAmountForDesiredDebt = async (desiredDebt) =>
    (await getOpenTroveBaseFeeLMAAmount(dec(desiredDebt, 18))).add(th.toBN(1));
  ...
  describe("Scale Factor issue tests", async () => {
    ...
    it("1. Liquidation succeeds after P reduced to 1", async () => {
      ...
      // Open 3 Troves with 2000 BaseFeeLMA debt
    ...
    it("2. New deposits can be made after P reduced to 1", async () => {
      ...
      // Whale gives BaseFeeLMA to D,E,F
      const newDeposits = [
        th.toBN(1),
      ...
      for (let i = 0; i < 3; i++) {
        await baseFeeLMAToken.transfer(newDepositors[i], newDeposits[i], {
          from: whale,
        });
        await stabilityPool.provideToSP(newDeposits[i], frontEnds[i], {
          from: newDepositors[i],
        });
        assert.isTrue(
          (
            await stabilityPool.getCompoundedBaseFeeLMADeposit(newDepositors[i])
          ).eq(newDeposits[i])
        );
      }
    });
    ...
    it("3. Liquidation succeeds when P == 1 and liquidation has newProductFactor == 1e9", async () => {
      ...
      const deposit_2 = deposit_0
        .sub(await stabilityPool.getTotalBaseFeeLMADeposits())
    ...
    it("5. Depositor have correct depleted stake after deposit at P == 1 and scale changing liq (with newProductFactor == 1e9)", async () => {
      ...
      // D makes deposit of 1000 BaseFeeLMA
      const D_deposit = dec(1, 21);
      await baseFeeLMAToken.transfer(D, dec(1, 21), { from: whale });
      ...
      const D_depletedDeposit =
        await stabilityPool.getCompoundedBaseFeeLMADeposit(D);
