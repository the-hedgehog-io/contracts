...
contract("BorrowerOperations", async (accounts) => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(): Increases the activePool WStETH and raw eth balance by correct amount", async () => {
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_WStETH, L_BaseFeeLMADebt snapshots", async () => {
      // --- SETUP ---
      ...
      const L_WStETH = await troveManager.L_WStETH();
      ...
      const alicePendingETHReward = await troveManager.getPendingWStETHReward(
        alice
      );
      const bobPendingETHReward = await troveManager.getPendingWStETHReward(
        bob
      );
      ...
      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_WStETH and L_BaseFeeLMADebt */
      ...
      assert.isAtMost(
        th.getDifference(alice_ETHrewardSnapshot_After, L_WStETH),
        100
      );
      assert.isAtMost(
        th.getDifference(
      ...
      assert.isAtMost(
        th.getDifference(bob_ETHrewardSnapshot_After, L_WStETH),
        100
      );
      assert.isAtMost(
        th.getDifference(
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 WStETH, withdraw 100/100/900 BaseFeeLMA
    ...
    //   // dennis tops up his trove by 1 WStETH
    ...
    //   the WStETH from her Trove has now become the totalPendingETHReward. So:

    //   totalStakes = (alice_Stake + bob_Stake + dennis_orig_stake ) = (15 + 4 + 1) =  20 WStETH.
    //   totalCollateral = (alice_Collateral + bob_Collateral + dennis_orig_coll + totalPendingETHReward) = (15 + 4 + 1 + 5)  = 25 WStETH.

    //   Therefore, as Dennis adds 1 eth collateral, his corrected stake should be:  s = 2 * (20 / 25 ) = 1.6 WStETH */
    ...
    it("withdrawColl(): reverts when requested WStETH withdrawal is > the trove's collateral", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });
    ...
    it("withdrawColl(): reduces ActivePool WStETH and raw eth by correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      const aliceCollBefore = await getTroveEntireColl(alice);
    ...
    it("withdrawColl(): sends the correct amount of WStETH to the user", async () => {
      await openTrove({
        ICR: toBN(dec(2, 18)),
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_WStETH, L_BaseFeeLMADebt snapshots", async () => {
      // --- SETUP ---
      ...
      const pendingCollReward_A = await troveManager.getPendingWStETHReward(
        alice
      );
      const pendingDebtReward_A =
        await troveManager.getPendingBaseFeeLMADebtReward(alice);
      const pendingCollReward_B = await troveManager.getPendingWStETHReward(
        bob
      );
    ...
    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      ...
      // Alice coll and debt increase(+1 WStETH, +50BaseFeeLMA)
      await borrowerOperations.adjustTrove(
        th._100pct,
        0,
        dec(50, 18),
    ...
    it("adjustTrove(): reverts in Recovery Mode when the adjustment would reduce the TCR", async () => {
      ...
      await priceFeed.setPrice(dec(120, 18)); // trigger drop in WStETH price
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      ...
      await priceFeed.setPrice(dec(105, 18)); // trigger drop in WStETH price
      const price = await priceFeed.getPrice();
    ...
    it("adjustTrove(): A trove with ICR < CCR in Recovery Mode can adjust their trove to ICR > CCR", async () => {
      ...
      await priceFeed.setPrice(dec(100, 18)); // trigger drop in WStETH price
      const price = await priceFeed.getPrice();
    ...
    it("adjustTrove(): reverts when attempted WStETH withdrawal is >= the trove's collateral", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } });
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } });
      ...
      // Carol attempts an adjustment that would withdraw 1 wei more than her WStETH
      try {
        const txCarol = await borrowerOperations.adjustTrove(
          th._100pct,
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      // Alice adjusts trove. Coll and debt increase(+1 WStETH, +50BaseFeeLMA)
      await borrowerOperations.adjustTrove(
        th._100pct,
        0,
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 WStETH, -50BaseFeeLMA)
      await borrowerOperations.adjustTrove(
        th._100pct,
        dec(500, "finney"),
        dec(50, 18),
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 WStETH, -50BaseFeeLMA)
      await borrowerOperations.adjustTrove(
        th._100pct,
        0,
        dec(50, 18),
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 WStETH, 10BaseFeeLMA)
      await borrowerOperations.adjustTrove(
        th._100pct,
        dec(1, 17),
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 WStETH, +50 BaseFeeLMA)
      await borrowerOperations.adjustTrove(
        th._100pct,
        0,
        dec(50, 18),
    ...
    it("adjustTrove(): Changes the activePool WStETH and raw eth balance by the requested decrease", async () => {
      await openTrove({
        extraBaseFeeLMAAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
    ...
    it("adjustTrove(): Changes the activePool WStETH and raw eth balance by the amount of WStETH sent", async () => {
      await openTrove({
        extraBaseFeeLMAAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
    ...
    it("closeTrove(): reduces ActivePool WStETH and raw eth by correct amount", async () => {
      await openTrove({
        extraBaseFeeLMAAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: dennis },
      ...
      // Check active Pool WStETH before
      const activePool_ETH_before = await activePool.getWStETH();
      const activePool_RawEther_before = toBN(
        await web3.eth.getBalance(activePool.address)
      );
    ...
    if (!withProxy) {
      ...
      it("closeTrove(): sends the correct amount of WStETH to the user", async () => {
        await openTrove({
          extraBaseFeeLMAAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: dennis },
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      // Carol's liquidated coll (1 WStETH) and drawn debt should have entered the Default Pool
    ...
    it("openTrove(): Increases the activePool WStETH and raw eth balance by correct amount", async () => {
      const activePool_ETH_Before = await activePool.getWStETH();
    ...
    it("openTrove(): records up-to-date initial snapshots of L_WStETH and L_BaseFeeLMADebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 eth, after liquidation, L_WStETH should equal 1/10 eth per-eth-staked,
       and L_BaseFeeLMA should equal 18 BaseFeeLMA per-eth-staked. */

      const L_WStETH = await troveManager.L_WStETH();
      const L_BaseFeeLMA = await troveManager.L_BaseFeeLMADebt();

      assert.isTrue(L_WStETH.gt(toBN("0")));
      ...
      // Check Bob's snapshots of L_WStETH and L_BaseFeeLMA equal the respective current values
      ...
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_WStETH), 1000);
      assert.isAtMost(
    ...
    if (!withProxy) {
      it("closeTrove(): fails if owner cannot receive WStETH", async () => {
        ...
        await th.assertRevert(
          ...
          "ActivePool: sending WStETH failed"
        );
      });
    }
  };
